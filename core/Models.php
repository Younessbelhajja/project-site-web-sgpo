...
class Models {
  ...
  const DB_PASS = '';
	 ...
	 */
	/*
	 * public function countligne() { $sql_ = "SELECT COUNT(*) from this->table" $res = $this->pdo->prepare($sql_); $res->execute(); foreach ( $res->fetchAll ( PDO::FETCH_ASSOC ) as $row ) { $arr_result [] = $row; } return $arr_result; }
	 */
	...
	function send_notification ($tokens, $message)
    {
      ...
      $headers = array(
        'Authorization:key =AAAAoBns0vE:APA91bG6K_RckwvJVtgWiFfoh8I1vuPEXGHieHO8WtJA2ZwwJER6DKPstKwcI1AcQyED40P51Wv0nYsilT8-KbjixZC4kgBiYYyQiV8vnusvRqyDG53mkH39ELgtyr2SLVYbhTGtbxZa',
    ...
    function save_action_journal($action,$table,$id_data){
		$this->table = "journal";
		$data['id_emp'] = $_SESSION ['user'] [0] ['Id_Emp'];
		$data['action'] = $action;
		$data['table'] = $table;
